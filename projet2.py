...
# print('Attributs choisis: types de crime -> i, arrestations -> j\n')
...
plt.scatter([0 for row in C_l], [row[0] for row in C_l])
plt.scatter([0 for row in C_c], [row[0] for row in C_c])

# Ajouter des etiquettes avec plt.annotate
for i in range(d):
    label_1 = "(" + L[i] + ")"
    plt.annotate(label_1, (0, C_l[i, 0]))

for i in range(e):
    label_2 = "(" + T[i] + ")"
    plt.annotate(label_2, (0, C_c[i, 0]), xytext = (0, C_c[i,0]), color = "Blue")
